...
srun rsync -av --progress --delete --exclude={'*.out','*.slurmlog'} "$NFS_DIR" "$LOCAL_DIR/"
...
if [[ -f "$LOCAL_DIR/$EXECUTABLE" ]]; then
	...
	perf stat -r 5 -- srun --nodes 2 --ntasks-per-node 4 -w soctf-pdc-015,soctf-pdc-008 --distribution=block "$LOCAL_DIR/$EXECUTABLE" "$LOCAL_DIR/$EXECUTABLE_ARGS" > $PROGRAM_OUTFILE 2>&1 
else
